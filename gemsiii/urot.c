...
void Qt_ToMatrix(Quat q, Matrix3 *out)
{
	...
	double s = (norm > 0.f) ? 2.f/norm : 0.f;
	double xs = q.x*s,		ys = q.y*s,		zs = q.z*s;
	double wx = q.w*xs,	wy = q.w*ys,	wz = q.w*zs,
		 	xx = q.x*xs,	xy = q.x*ys,	xz = q.x*zs,
		 	yy = q.y*ys,	yz = q.y*zs,	zz = q.z*zs;
	double (*mat)[3] = out->element;
	mat[X][X] = 1.f - (yy + zz); mat[X][Y] = xy + wz; mat[X][Z] = xz - wy;
	mat[Y][X] = xy - wz; mat[Y][Y] = 1.f - (xx + zz); mat[Y][Z] = yz + wx;
	mat[Z][X] = xz + wy; mat[Z][Y] = yz - wx; mat[Z][Z] = 1.f - (xx + yy);
} /* Qt_ToMatrix */
 ...
 */
	...
	INT32 seed = *argseed;
#define uurand() ((seed = (seed+1)*69069)/2147483648.f)
	float x = uurand(), y = uurand();
	float z = uurand(), w = uurand();
	float s1, s2;
	float num1, num2, root1, root2, r;
	while ((s1 = x*x+y*y) > 1.f) {x = uurand(); y = uurand();}
	while ((s2 = z*z+w*w) > 1.f) {z = uurand(); w = uurand();}
	/* Now the point (x,y) is uniformly distributed in the unit disk */
	/* So is the point (z,w), independently */
	num1 = -2*logf(s1); num2 = -2*logf(s2);
	...
	root1 = sqrtf((num1/s1)/r); root2 = sqrtf((num2/s2)/r);
 ...
 */
	Quat q;
	float r1 = sqrtf(1.f - x[0]), r2 = sqrtf(x[0]);
	float t1 = PITIMES2*x[1], t2 = PITIMES2*x[2];
 ...
 */
void M3_RandomRot(float x[3], Matrix3 *m)
{
