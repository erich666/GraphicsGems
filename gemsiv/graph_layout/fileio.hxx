...
public:
    FileIO(const char * op ) { strcpy( operation, op ); }
	BOOL	OpenFile ( cpchar );	    // open file having that name
	BOOL	GetString ( pchar, int );   // get a string from the file
	BOOL	GetKeyWord ( cpchar );	    // get a string and compare to a key
	BOOL	GetKeyAgain ( cpchar );	    // get last string again and compare
	...
	BOOL	PutString ( cpchar );	    // write string to the file
